...
import { Mail, ShieldAlert, MailCheck, ChevronRight, AlertTriangle } from 'lucide-react'
...
export default function ForgotPassword() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Don't reveal whether an account exists (prevents email enumeration) — the
    // only error worth surfacing is the rate-limit, which is actionable.
          ...
          {sent ? (
            <div className="text-center py-3">
              ...
              <p className="text-slate-400 text-sm leading-relaxed mb-6">
                ...
                <span className="text-white font-medium break-all">{email}</span>, we&apos;ve sent a
                password-reset link. It expires in 1 hour.
              </p>
              <div className="flex items-start gap-2.5 text-left bg-amber-500/10 border border-amber-500/25 rounded-lg px-3.5 py-3 mb-6">
                <AlertTriangle className="w-4 h-4 text-amber-400 mt-0.5 shrink-0" />
                <p className="text-amber-200/90 text-xs leading-relaxed">
                  Don&apos;t see it? <span className="font-semibold text-amber-100">Check your spam / junk folder</span> — and tap &quot;Not spam&quot; so the reset link opens normally.
                </p>
              </div>
              <Link
                href="/login"
            ...
            <>
              ...
              <div className="mt-7 pt-5 border-t border-white/[0.06] text-center">
                <p className="text-slate-400 text-sm">
                  ...
                  <Link
                    href="/login"
                    className="text-accent-emerald font-semibold hover:brightness-125 transition-all"
                  >
                    Back to login
                  </Link>
                </p>
