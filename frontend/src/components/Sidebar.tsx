...
export function Sidebar({ children }: { children: React.ReactNode }) {
  ...
  return (
    <>
      ...
      <aside
        className="app-shell fixed left-0 top-0 bottom-0 z-40
                   flex flex-col py-4
      ...
      >
        ...
        <div className={`flex items-center mb-5 ${collapsed ? 'justify-center' : 'justify-between px-3'}`}>
          <Link
            href="/dashboard"
        ...
        {/* Collapse toggle when collapsed — separate row underneath brand */}
        {collapsed && (
          <button
        ...
        {/* Quick Search trigger — no hint chip */}
        <NavRow
          collapsed={collapsed}
        ...
        {/* Divider */}
        <div className={`h-px bg-white/[0.06] my-3 ${collapsed ? 'w-10 self-center' : 'mx-3'}`} />
      ...
      {!panelOpen && (
        <button
          ...
          className="app-shell fixed top-4 right-4 z-40
      ...
      <div
        ...
        style={{ paddingLeft: sidebarWidth, paddingRight: panelWidth }}
      >
        {children}
      </div>
